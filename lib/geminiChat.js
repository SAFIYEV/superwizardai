/** Shared LLM REST helpers (Express, Vite middleware, Cloudflare Worker). */

export const CHAT_MODEL_ID = 'z-ai/glm4.7';

export const ALLOWED_MODELS = new Set([CHAT_MODEL_ID]);

export const GEMINI_API_BASE = 'https://integrate.api.nvidia.com/v1';

/**
 * Authorization header for OpenAI-compatible APIs.
 * @param {string} apiKey
 */
...
export function buildGeminiGenerateBody(sanitized) {
  ...
  if ((sanitized.model || CHAT_MODEL_ID) === 'z-ai/glm4.7') {
...
export async function geminiFetchWithRetry(url, init, maxRetries = 5, timeoutMs = 45000) {
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      if (err && typeof err === 'object' && err.name === 'AbortError') {
        return new Response(
          JSON.stringify({ error: { message: `Upstream timeout after ${timeoutMs}ms` } }),
